// index.js
const { Client, GatewayIntentBits, ActivityType, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, SectionBuilder, TextDisplayBuilder, ThumbnailBuilder, SeparatorBuilder, SeparatorSpacingSize, MessageFlags, StringSelectMenuBuilder } = require('discord.js');
...
async function deleteNowPlayingMessage(guildId) {
...
function getGuildVolume(guildId) {
  if (!guildVolumes.has(guildId)) {
    guildVolumes.set(guildId, 100);
  }
  return guildVolumes.get(guildId);
}
...
function createNowPlayingContainer(player, track, disabled = false) {
  ...
  const volume = getGuildVolume(player.guildId);
  ...
  const container = new ContainerBuilder()
      ...
      new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder()
            .setContent(`## MUSIC PANEL\n${config.emojis.music} [${info.title || 'Unknown Title'}](${info.uri || 'https://youtube.com'})`)
        )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder()
        .setContent(`**Requested By:** <@${track.info.requester}>\n**Music Duration:** ${formatTime(totalDuration)}\n**Music Author:** ${info.author || 'Unknown'}`)
    )
    .addTextDisplayComponents(
      new TextDisplayBuilder()
        .setContent(`**Status:** ${status} • **Volume:** ${volume}%\n**Progress:** ${formatTime(position)} / ${formatTime(totalDuration)}\n**Loop:** ${loop}`)
    )
...
function createQueueContainer(player, guild, user) {
  ...
  if (current?.info) {
    options.push({
      label: truncate(current.info.title, 50),
      description: 'Now Playing',
      value: 'current',
      emoji: '▶️'
    });
  ...
  queue.forEach((t, i) => {
    const inf = t.info || {};
    options.push({
      label: truncate(inf.title, 50),
      description: `${inf.author || 'Unknown'} • ${formatTime(inf.length || 0)}`,
      value: String(i),
      emoji: '🎵'
    });
  });
  ...
  let builder = new ContainerBuilder()
    .addSectionComponents(
      new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder()
            .setContent(`## Playlist Panel\n${config.emojis.queue} Playlist's\n\n${description}`)
        )
        .setThumbnailAccessory(
          new ThumbnailBuilder()
            .setURL(thumbnail)
            .setDescription('Playlist Panel')
        )
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
  ...
  if (options.length > 0) {
    const selectMenu = new StringSelectMenuBuilder()
      ...
      .setPlaceholder('Pilih lagu di playlist')
      .setMinValues(1)
      .setMaxValues(1)
      .addOptions(options);

    builder = builder.addActionRowComponents(
...
client.on('interactionCreate', async (interaction) => {
    ...
    switch (interaction.customId) {
      ...
      case 'shuffle': {
        if (player.queue.length === 0) {
          return interaction.reply({ content: `${config.emojis.error} Queue is empty`, flags: MessageFlags.Ephemeral });
        }
      ...
      case 'autoplay': {
        ...
        if (queue247.has(guildId)) {
          queue247.delete(guildId);
        } else {
          queue247.add(guildId);
        }
        await interaction.deferUpdate();
        break;
      }
      ...
      case 'queue': {
        ...
        await interaction.reply({ components: [queueContainer], flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral });
        break;
      }
      case 'volume_down':
      case 'volume_up': {
        let currentVolume = getGuildVolume(player.guildId);
        if (interaction.customId === 'volume_down') {
          currentVolume -= 10;
        } else {
          currentVolume += 10;
        }
        const newVolume = applyVolume(player, currentVolume);
        const message = nowPlayingMessages.get(player.guildId);
        if (message && player.current) {
          const updatedContainer = createNowPlayingContainer(player, player.current);
          await message.edit({ components: [updatedContainer], flags: MessageFlags.IsPersistent | MessageFlags.IsComponentsV2 }).catch(() => {});
  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId === 'queue_select') {
      ...
      if (value === 'current') {
        ...
        return interaction.reply({ content: `${config.emojis.info} Already playing this track`, flags: MessageFlags.Ephemeral });
      }

      const index = parseInt(value, 10);

      if (Number.isNaN(index) || index < 0 || index >= player.queue.length) {
        return interaction.reply({ content: `${config.emojis.error} Invalid track selected`, flags: MessageFlags.Ephemeral });
      }

      const target = player.queue.remove(index);
      player.queue.splice(0, 0, target);
      ...
      await interaction.reply({ content: `${config.emojis.success} Playing: ${target.info.title}`, flags: MessageFlags.Ephemeral });
    }
    return;
  }
  ...
  if (commandName === 'play') {
    ...
    if (!isLavalinkConnected) {
      return interaction.reply({ content: `${config.emojis.error} Lavalink is not connected. Music commands are unavailable.`, flags: MessageFlags.Ephemeral });
    }
    ...
    try {
      ...
      if (!player) {
        ...
        applyVolume(player, getGuildVolume(guild.id));
      }
  ...
  if (commandName === 'pause') {
    ...
    if (!player) return interaction.reply({ content: `${config.emojis.error} No player found`, flags: MessageFlags.Ephemeral });
    if (!member.voice.channel || member.voice.channel.id !== player.voiceChannel) {
      return interaction.reply({ content: `${config.emojis.error} You need to be in the same voice channel`, flags: MessageFlags.Ephemeral });
    }
  ...
  if (commandName === 'stop') {
    ...
    await deleteNowPlayingMessage(guild.id);
    player.destroy();
  ...
  if (commandName === 'volume') {
    ...
    const applied = applyVolume(player, volume);
    const container = createSimpleContainer('Volume Set', `Volume set to ${applied}%`, config.emojis.volume);
    await interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
  }
  ...
  if (commandName === 'nowplaying') {
    ...
    const volume = getGuildVolume(guild.id);
      ...
      if (config.enablePrefix) {
        client.on('messageCreate', async (message) => {
          ...
          if (command === 'play') {
            ...
            applyVolume(player, getGuildVolume(message.guild.id));
              }

              const resolve = await riffy.resolve({ query: searchQuery, requester: message.author.id });
          ...
          await deleteNowPlayingMessage(message.guild.id);
            player.destroy();
            const container = createSimpleContainer('Stopped', 'Stopped and cleared queue', config.emojis.stop);
            await message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
          }
          ...
          if (command === 'nowplaying') {
            ...
            const totalDuration = info.length || 0;
            const status = player.paused ? '⏸️ Paused' : '▶️ Playing';
            const volume = getGuildVolume(message.guild.id);
